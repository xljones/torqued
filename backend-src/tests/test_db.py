...
def test_database_url_pins_psycopg_driver_on_provider_urls(
    ...
    # Hosted providers (a managed Postgres, Heroku, …) hand out driverless URLs; pin
    # psycopg v3 so they can be pasted in verbatim, query string (?sslmode=…) preserved.
    monkeypatch.setenv("DATABASE_URL", "postgres://u:p@db.example.com/db?sslmode=require")
    assert db.database_url() == "postgresql+psycopg://u:p@db.example.com/db?sslmode=require"
    monkeypatch.setenv("DATABASE_URL", "postgresql://u:p@db.example.com/db")
    assert db.database_url() == "postgresql+psycopg://u:p@db.example.com/db"
