...
def get_tax(vehicle_id: int) -> ResponseReturnValue:
    with get_db() as db:
        ...
        return jsonify(
            ...
            tax=VesRepository(db).get_for_vehicle(vehicle_id),
        ), 200
...
def refresh_tax(vehicle_id: int) -> ResponseReturnValue:
    ...
    if not ves.is_configured():
        return jsonify(error="DVLA Vehicle Enquiry Service API key is not configured"), 503
    try:
        payload = ves.fetch_vehicle(registration)
