...
describe('NeonStats', () => {
  it('renders storage and compute as a % of their limits once stats load', async () => {
    api.getNeonStats.mockResolvedValue({
      ...
      cpu_limit_seconds: 360000, // 100 hrs → 2%
    ...
    await waitFor(() => {
      ...
      expect(screen.getByText('2.0% used')).toBeInTheDocument();
      expect(screen.getByText('2.00 hrs / 100 hrs')).toBeInTheDocument();
      expect(screen.getByText(/torqued-db/)).toBeInTheDocument();
    });
  });

  it('falls back to plain figures when no limits are returned', async () => {
    api.getNeonStats.mockResolvedValue({
      configured: true,
