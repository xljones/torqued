...
- **Dashboard** — stat tiles, maintenance reminders, and "The garage": a per-vehicle table of odometer, front/rear tyre pressures (there is no separate tyre-pressures page) and service count.
- **VehicleList** — vehicle cards grouped by kind, with a persisted "Show archived" toggle (`torqued.showArchived`).
