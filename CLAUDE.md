...
- **Reminders** are derived, not stored: any service log with a `next_due_*` value creates one until a newer log in the same category exists. Status is `overdue` / `due_soon` (≤30 days or ≤500 km away) / `upcoming`. Reminders are tagged `type` (`service`); a vehicle's **MOT** also surfaces here (`type='mot'`, no `upcoming`): `MotRepository.reminders` emits an `overdue` (lapsed) or `due_soon` reminder once the MOT expiry — the latest test's `expiry_date`, falling back to the DVSA `mot_test_due_date` — is within `MOT_DUE_SOON_DAYS` (~2 months). Road **tax** works the same way (`type='tax'`): `TaxRepository.reminders` emits `overdue`/`due_soon` from the stored `tax_due_date` (within `TAX_DUE_SOON_DAYS`, ~1 month), while SORN/untaxed vehicles carry no due date and so raise no reminder. `ServiceLogRepository.reminders` merges the service, MOT and tax streams so all three appear wherever reminders do (dashboard, vehicle detail, PDF report).
...
- **Road tax / SORN (UK)** — `torqued/tax.py` resolves a vehicle's **tax status**, **SORN**, and **tax due date**. The official DVLA Vehicle Enquiry Service (VES) API is closed to sign-ups, so this **scrapes** the public gov.uk "Check if a vehicle is taxed" service (`vehicleenquiry.service.gov.uk`) — no credentials, opt out with `VES_SCRAPE_ENABLED=0` — behind the same `fetch_tax(reg)` shape the VES API would have, so it's a drop-in swap when sign-ups reopen. Tax lookups are stored in `vehicle_tax` with the same **retain-as-history** shape as `dvsa_vehicles` (migration `0006` gave it a surrogate `id` + nullable `vehicle_id` / `ON DELETE SET NULL`): each refresh keeps the prior lookup detached, records survive vehicle deletion, and `TaxRepository.relink_detached` re-ties a plate's records to a vehicle added later — mirroring `MotRepository`. `POST /api/vehicles/<id>/tax/refresh` (write roles) fetches + stores; `GET /api/vehicles/<id>/tax` reads the live snapshot; `GET /api/tax/status` gates the UI. Tax is folded into the vehicle-detail **MOT & tax** card and fetched only on vehicle create or an explicit refresh (never per-view). See [docs/TAX_API.md](docs/TAX_API.md).
